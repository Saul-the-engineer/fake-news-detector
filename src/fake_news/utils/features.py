...
def normalize_labels(datapoints: List[Dict]) -> List[Dict]:
    ...
    for datapoint in datapoints:
        ...
        # Check if the label exists and is in the expected format
        if label is not None:
            label_cleaned = label.lower().strip()
            if label_cleaned in SIX_WAY_LABEL_TO_BINARY:
                normalized_datapoint["label"] = SIX_WAY_LABEL_TO_BINARY[label_cleaned]
            else:
                # Handle unexpected label values (e.g., log, skip, or assign a default)
                normalized_datapoint["label"] = None  # Or choose a default value
        else:
            # Handle the case where the label is missing
            normalized_datapoint["label"] = None
...
def normalize_and_clean_speaker_title(datapoints: List[Dict]) -> List[Dict]:
    ...
    for datapoint in datapoints:
        # First do simple cleaning
        normalized_datapoint = deepcopy(datapoint)
        old_speaker_title = normalized_datapoint.get("speaker_title")
        
        # Check if speaker_title is not None before processing
        if old_speaker_title is not None:
            ...
            # Handle the None case, e.g., set to "Unknown" or another default value
...
def normalize_and_clean_state_info(datapoints: List[Dict]) -> List[Dict]:
    ...
    for datapoint in datapoints:
        normalized_datapoint = deepcopy(datapoint)
        old_state_info = normalized_datapoint.get("state_info")
        
        # Check if state_info is not None before processing
    ...
    for idx, datapoint in enumerate(datapoints):
        ...
        for count_col in [
            ...
            if count_col in normalized_datapoint:
                count_value = normalized_datapoint[count_col]
                # Check if the count value is not None before converting to float
                if count_value is not None:
                    normalized_datapoint[count_col] = float(count_value)
                else:
                    # Handle the None case, e.g., set to 0 or another default value
